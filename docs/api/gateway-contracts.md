...
`SessionType=MEMBER`：验签 JWT 后写入 `memberId`（`X-MEMBER-ID`），跳过 DPoP/摘要，入口走 `MemberPerm(organId)` 且要求 `organId`+`memberId`。详见 [MEMBER 会话入口处理](../design/member-session-gateway.md)。
