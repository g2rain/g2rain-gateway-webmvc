# g2rain-gateway-webmvc 工程文档

本项目以 `webmvc` 实现类型计划接入中央 [`gateway-service 1.0.0-draft`](https://github.com/g2rain/g2rain/tree/feature/g2rain-architectur-init/docs/architecture/profiles/gateway-service)。

## 架构

- [架构总览](architecture/overview.md)
- [模块](architecture/modules.md)
- [依赖](architecture/dependencies.md)
- [运行流程](architecture/runtime-flows.md)
- [偏差](architecture/deviations.md)

## 工程与运行

- [本地开发](development/local-development.md)
- [代码约定](development/code-conventions.md)
- [测试](development/testing.md)
- [完成定义](development/definition-of-done.md)
- [Gateway 契约](api/gateway-contracts.md)
- [MEMBER 会话入口处理](design/member-session-gateway.md)
- [MEMBER 接口权限校验升级](design/member-api-permission-upgrade.md)
- [配置](operations/configuration.md)
- [部署](operations/deployment.md)
- [故障排查](operations/troubleshooting.md)
- [安全边界](security/security-boundaries.md)
- [需求](requirements/README.md)
- [决策](decisions/README.md)
