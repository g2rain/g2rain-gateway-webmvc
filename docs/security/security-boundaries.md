...
- `SessionType=MEMBER`：JWT 写入 `memberId`（`X-MEMBER-ID`），跳过 DPoP/摘要；入口权限按租户已开通的 MEMBER 控制单元（`MemberPerm`），要求 `organId`+`memberId`；不按 `userId` 走员工权限。专题：[MEMBER 会话入口处理](../design/member-session-gateway.md)、[MEMBER 接口权限](../design/member-api-permission-upgrade.md)。
